...
export default function decipher(this: Mocha.Suite): void {
  ...
  it('should decipher encrypted email from pst', async function () {
    ...
    const decipherRes: ChaiHttp.Response = await chai.request(apiURL).post('/decipher').send({caseId, password})
    ...
    const delRes: ChaiHttp.Response = await chai.request(apiURL).delete(`/decipher/upload/pst/${caseId}`)
    expect(delRes).to.have.status(200)
    expect(fs.readdirSync(`/app/workspace/${caseId}/ctPSTs`).length).to.eq
    // run it again without pw
    const noPass: ChaiHttp.Response = await chai.request(apiURL).post('/decipher').send({caseId})
    expect(noPass).to.have.status(403)
  })
  it('FAILS to upload/delete ct PST with BAD caseId', async function(){
    const res404_upload: ChaiHttp.Response = await chai.request(apiURL).post('/decipher/upload/pst/FUBAR')
    expect(res404_upload).to.have.status(404)
    const res500_del: ChaiHttp.Response = await chai.request(apiURL).delete('/decipher/upload/pst/FUBAR')
