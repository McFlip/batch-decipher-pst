import Keys from 'pages/[caseId]/keys'
...
describe('Keys Pg', () => {
	it('renders a serial # from props', () => {
		const { getByText, queryByText } = render(<Keys serialsProp={['1a2b3c']} caseId='1' />)
	...
	it('extracts a key from a p12', async () => {
		const { getByLabelText, getByRole, findByText } = render(<Keys serialsProp={['']} caseId='1' />)
		await userEvent.upload(getByLabelText(/select one p12/i), testFile )
		// check missing key pw
		await userEvent.type(getByLabelText('Enter password for p12 file:'), 'p12-password')
		...
		await userEvent.type(getByLabelText('Use a password manager to create a new password for the extacted key:'), 'key-password')
		await userEvent.click(getByRole('button', { name: 'Upload' }))
		expect(alertMsg[1]).toBe("Please enter the password for the p12 container")
		// do it right
		await userEvent.type(getByLabelText('Enter password for p12 file:'), 'p12-password')
		await userEvent.click(getByRole('button', { name: 'Upload' }))
