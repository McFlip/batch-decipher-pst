...
describe('File Upload Component', () => {
	it('alerts the user when deleting uploads fails', async () => {
		...
		const { getByText } = render(<Uploader caseId='12345' fileType='pst' destination='sigs' files={files} setFiles={setFiles} />)
		// check delete button
	...
	it('validates a pst upload', async () => {
		const { getByText, getByRole, getByLabelText } = render(<Uploader caseId='12345' fileType='pst' destination='sigs' files={files} setFiles={setFiles} />)
		...
		await userEvent.upload(getByRole('button',{ name: 'File Upload' }), new File(['not a pst'], 'test.jpg') )
		await act(() => userEvent.click(getByText('Upload')))
	...
	it('validates a p12 upload', async () => {
		...
		expect(getByLabelText('Use a password manager to create a new password for the extacted key:')).toBeInTheDocument()
		// clicking upload before selecting files should cause error msg
		await act(() => userEvent.click(getByText('Upload')))
		expect(alertMsg[1]).toBe('Please select pst to upload')
		// uploading anything other than p12 files should fail
		await userEvent.upload(getByRole('button',{ name: 'File Upload' }), new File(['not a p12'], 'test.jpg') )
		await act(() => userEvent.click(getByText('Upload')))
		expect(alertMsg[2]).toBe('Check file upload type')
		// do it right this time, check file mock for success
