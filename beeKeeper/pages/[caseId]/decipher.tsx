...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  try {
    const resSerials = await fetch(urlKeys, {
      method: 'GET',
      mode: 'cors',
      cache: 'default',
      headers: {'Content-Type': 'application/json'}
    })
    const serials = resSerials.ok ? await resSerials.json() : null
    return {
...
export default function Decipher ({ serialsProp, caseId }: propsType) {
  ...
  const handleRun = async () => {
    if(!secrets) return alert('missing password')
    setIsRunning(true)
  ...
  return(
    <div className='container'>
      ...
      <main>
        ...
        { !serialsProp ? <div className='alert alert-danger'>WARNING: no keys are loaded for this case!</div> : '' }
        ...
        <hr/>
        <h2>Launch Script</h2>
        <button className='btn btn-primary' disabled={!serialsProp || isRunning} onClick={() => handleRun()}>
          { isRunning ?
